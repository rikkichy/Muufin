...
class JfMusicApp extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Material 3 Expressive (Flutter doesn't ship the token refresh yet), but we can
    // still apply the tactics + dynamic color/harmonization.
    const seed = Color(0xFF6750A4);

    return DynamicColorBuilder(
      builder: (lightDynamic, darkDynamic) {
        final lightScheme = (lightDynamic ?? ColorScheme.fromSeed(seedColor: seed, brightness: Brightness.light))
            .harmonized();
...
/// Makes GoRouter re-evaluate redirects when a Riverpod stream changes.
